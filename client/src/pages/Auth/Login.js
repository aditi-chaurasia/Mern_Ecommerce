...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.post(`${process.env.REACT_APP_API}/api/user/Login`, { email, password });
      if (res && res.data.success) {
        ...
        // Redirect to previous location or home if not available
  ...
  return (
    <Layout title="Login - TrendyCart">
      <section className="py-2 bg-gray-50 sm:py-5 lg:py-20">
        <div className="px-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
          <div className="max-w-2xl mx-auto text-center">
            <h2 className="text-2xl font-bold leading-tight text-black sm:text-4xl lg:text-5xl">Login to your account</h2>
            <p className="max-w-xl mx-auto mt-2 text-base leading-relaxed text-gray-600">Please enter your credentials to log in</p>
          </div>
              ...
              <div className="px-4 py-2 sm:px-8 sm:py-7">
                <form onSubmit={handleSubmit}>
                  <div className="space-y-2.5">
                    ...
                    <div>
                      <button type="submit" className="inline-flex items-center justify-center w-full px-4 py-2 text-base font-semibold text-white transition-all duration-200 bg-blue-600 border border-transparent rounded-md focus:outline-none hover:bg-blue-700 focus:bg-blue-700" onClick={()=>{
